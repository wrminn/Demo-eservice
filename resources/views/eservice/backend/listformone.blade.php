    ...
    <div class="card">
            ...
            <table class="table caption-top" id="myTable">
                            ...
                            @if (!$form->isEmpty())
                                <select class="form-select" aria-label="Default select example"
                                    ...
                                    @foreach ($form as $item)
                                        <option value="{{ route('eserviceform.id', ['id' => $item->gennerricforms_id]) }}"
                                            {{ $item->gennerricforms_id == $id ? 'selected' : '' }}>
                                            {{ $item->gennerricforms_name }}
                                        </option>
                                    @endforeach
                                </select>
                            @endif
                ...
                <thead>
                    <tr>
                        ...
                        <th scope="col"class="col-2" style="width: 20%;">ผู้ยื่นแบบฟอร์ม</th>
                        <th scope="col"class="col-2" style="width: 20%;">สถานะ</th>
                    ...
                    @else
                        @foreach ($list as $item)
                            <tr>
                                ...
                                <td>
                                    {{ $NameUser[$item->form_user_id] ?? 'ไม่ระบุตัวตน' }}
                                </td>
                                <td>
                                    ...
                                    @elseif ($item->form_status === 'N')
                                        <span class="badge bg-secondary rounded-pill px-3 py-2">
                                            ● กำลังขอข้อมูลเพิ่มเติม
                                        </span>
                                    @elseif ($item->form_status === 'M')
                                        <span class="badge bg-warning rounded-pill px-3 py-2">
                                            ● มีข้อมูลเพิ่มเติมจากประชาชน
                                        </span>
                                    @endif
                                </td>
                                <td>
                                    ...
                                    @endif
                                    @if ($item->form_status != 'C')
                                        <button type="button" class="btn btn-success btn-sm btn-reply-nouser"
        ...
        <div class="modal fade" id="replyModal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <form id="replyForm" method="POST" action="{{ route('eservice.replynouser') }}">
                        @csrf
                        <div class="modal-header">
                            <h5 class="modal-title">แนบหมายเหตุ</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                        </div>
                        <div class="modal-body">
                            <input type="hidden" name="form_id" id="modal-form-nouser-id">
                            <input type="hidden" name="form_table" id="modal-form-table">
                            <div class="mb-3">
            ...
            document.addEventListener('DOMContentLoaded', function() {
                document.querySelectorAll('.btn-reply').forEach(btn => {
                    btn.addEventListener('click', function() {
                        ...
                        const texteditorId = this.dataset.form;
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                document.querySelectorAll('.btn-reply-nouser').forEach(btn => {
                    btn.addEventListener('click', function() {
                         const texteditorId = this.dataset.id;
                        const texteditorIdform = this.dataset.form;
                        document.getElementById("modal-form-nouser-id").value = texteditorId;
                        document.getElementById('modal-form-table').value = texteditorIdform;
                    });
                });
            });
        </script>
